...
namespace Lab3_QLKhachSan.Services
{
    public class TraPhongService
    {
        public DataTable LayPhieuDangO() =>
            Db.Query("SELECT d.SoPhieuDat, k.HoTen, d.NgayNhanThucTe, d.NgayTraDuKien FROM PhieuDatPhong d JOIN KhachHang k ON d.MaKhach = k.MaKhach WHERE d.TrangThai = N'Đang ở' ORDER BY d.SoPhieuDat");

        public DataTable LayPhongTheoPhieu(string so) =>
            Db.Query("SELECT c.SoPhong, p.DonGiaNgay FROM ChiTietDatPhong c JOIN Phong p ON c.SoPhong = p.SoPhong WHERE c.SoPhieuDat = @s", new SqlParameter("@s", so));

        public DataTable LayTienNghiPhong(string phong) =>
            Db.Query(@"SELECT TOP 100 p.MaTienNghi, l.TenLoaiTN, t.TinhTrangHienTai 
                     FROM PhieuLapDat p 
        ...
        public DataTable LayQuyDinh() =>
            Db.Query("SELECT q.*, l.TenLoaiTN FROM QuyDinhDenBu q JOIN LoaiTienNghi l ON q.MaLoaiTN = l.MaLoaiTN ORDER BY l.TenLoaiTN, q.MucDoThietHai");

        public DataTable LayHoaDon() =>
            Db.Query("SELECT h.*, k.HoTen FROM HoaDon h JOIN PhieuDatPhong d ON h.SoPhieuDat = d.SoPhieuDat JOIN KhachHang k ON d.MaKhach = k.MaKhach ORDER BY h.NgayLap DESC");
        ...
        public KetQuaXuLy LapHoaDon(string soHD, string soDat, DateTime ngay, string maNV, int soNgay)
        {
            ...
            try
            {
                decimal phong = Convert.ToDecimal(Db.Scalar(@"SELECT ISNULL(SUM(p.DonGiaNgay), 0) 
                                                             FROM ChiTietDatPhong c 
                                                             JOIN Phong p ON c.SoPhong = p.SoPhong 
                                                             WHERE c.SoPhieuDat = @s", new SqlParameter("@s", soDat))) * soNgay;

                decimal dv = Convert.ToDecimal(Db.Scalar(@"SELECT ISNULL(SUM(c.ThanhTien), 0) 
                                                         FROM PhieuSuDungDV h 
                                                         JOIN ChiTietPhieuSuDungDV c ON h.SoPhieuSDDV = c.SoPhieuSDDV 
                                                         WHERE h.SoPhieuDat = @s", new SqlParameter("@s", soDat)));
                ...
                return KetQuaXuLy.Ok("Đã lập hóa đơn thành công.");
            }
        ...
        public KetQuaXuLy TraPhong(string soDat, DateTime ngayTra)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        if (!rd.Read()) return KetQuaXuLy.Fail("Chưa lập hóa đơn cho phiếu đặt này.");
                    ...
                    tx.Commit();
                    return KetQuaXuLy.Ok("Đã hoàn tất trả phòng và giải phóng phòng thành Trống.");
                }
                catch (Exception ex)
                {
                    try { tx.Rollback(); } catch { }
